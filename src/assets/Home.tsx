    ...
    return (
        <div className="w-full" style={{ height: "calc(100vh - 64px)" }}>
            <div className="relative w-full h-full overflow-hidden bg-black">
                {/* Dark overlay */}
                <div className="absolute inset-0 bg-gradient-to-b from-black/20 to-black/65 z-10 pointer-events-none" />

                {/* Fill top gap */}
                <div className="absolute top-0 left-0 right-0 bg-slate-900 z-20 pointer-events-none" style={{ height: "4%" }} />

                {/* Top shadow */}
                <div className="absolute left-0 right-0 bg-gradient-to-b from-slate-900 to-transparent z-20 pointer-events-none" style={{ top: "4%", bottom: "40%" }} />

                {/* Bottom shadow */}
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-900 to-transparent z-20 pointer-events-none" style={{ top: "70%" }} />
                ...
                <div className="absolute inset-0 z-30 flex flex-col items-center justify-center text-center px-4">
                    ...
                    <div className="mt-10 grid grid-cols-4 gap-4 w-full max-w-7xl">
                        {whyUs.map(({ icon: Icon, title, desc, extraIcon, extraIcons }) => (
                            <div
                                key={title}
                                className="bg-slate-900/70 backdrop-blur-sm border border-slate-700/50 rounded-xl p-5 flex flex-col items-center text-center"
                            >
                                ...
                                {extraIcon && (
                                    <img src={extraIcon} alt="WhatsApp" className="w-10 h-10 p-2 mt-2 bg-green-700 rounded-xl border-green-600 border-2
